...
func ProbeDns(data map[string]interface{}, target string) (resultData map[string]interface{}) {

	log.Println("Probing Dns Start: ", target)
	...
	result := bep.ProbeDNS(ctx, target,
		bec.Module{Timeout: time.Microsecond, DNS: t}, registry, logger.NewNopLogger())
